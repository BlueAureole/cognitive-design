 ...
 * Msg的dao方法缓存
 */
public class MsgMethodCache extends DaoMethodCache{
	private final String genericPTypeName="P";

	public MsgMethodCache(Class<?> currentClass) {
		super(currentClass);
	}
	...
	protected boolean isSubCollectionMethodMatch(Method method) {
        ...
        returnTypeMatch = ReflectGenericUtils.isReturnExactMessageListMatchGeneric(method, this.genericPType,genericPTypeName);
    ...
    /**
     * add 方法匹配规则：前缀add/insert + 入参List<P> + 返回Message<Long>
     */
	@Override
	protected boolean isAddMethodMatch(Method method) {
        ...
        boolean isReturnMatch = ReflectGenericUtils.isReturnMessageLongMatch(method);  // 返回值匹配
    ...
    /**
     * update 方法匹配规则：前缀update + 入参List<P> + 返回Message<Long>
     */
	@Override
    ...
    /**
     * delete 方法匹配规则：前缀delete + 入参List<P> + 返回Message<Long>
     */
	@Override
